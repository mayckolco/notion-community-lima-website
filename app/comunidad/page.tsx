...
import { listComunidadMembers } from "@/lib/notion/comunidad";
...
export default async function ComunidadPage() {
  const members = await listComunidadMembers();
        ...
        <div className="max-w-5xl mx-auto space-y-10">
            ...
            <section className="space-y-4" aria-labelledby="lista-comunidad">
              ...
              <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {members.map((member) => (
                  <li
                    key={member.id}
                    className="rounded-xl border border-border bg-card p-4 shadow-soft space-y-1"
                  >
                    <p className="font-serif">{member.nombre}</p>
                    {member.rol && (
                      <p className="text-xs text-primary font-mono">{member.rol}</p>
                    )}
                    {member.ciudad && (
                      <p className="text-sm text-muted-foreground">{member.ciudad}</p>
                    )}
                  </li>
                ))}
              </ul>
          ...
          <section className="rounded-xl border border-border bg-card p-6 sm:p-8 shadow-soft space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                Registrarme en el mapa
              </Link>
              <JoinCommunityButton location="comunidad_cta" size="default" variant="outline" />
              <Link
                href="/directorio"
                className="inline-flex items-center justify-center gap-1 min-h-[44px] px-4 text-sm font-medium text-primary hover:underline touch-manipulation"
              >
